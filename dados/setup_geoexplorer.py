# -*- coding: utf-8 -*-
"""
Setup do GeoExplorer no Appwrite 1.8.0+ / TablesDB.
Cria as tabelas, define as colunas e popula as 30 trilhas da DIO.
"""
...
for table_id, table_name in tabelas.items():
    try:
        db.create_table(database_id=db_id, table_id=table_id, name=table_name)
        print(f"Tabela '{table_id}' criada!")
    except Exception as e:
        print(f"Nota tabela '{table_id}': {e}")

# ──────────────────────────────────────────────
# 2. Colunas da tabela 'trilhas'
# ──────────────────────────────────────────────
colunas_trilhas = [
...
for key, tipo, size, required in colunas_trilhas:
    try:
        if tipo == "string":
            db.create_string_column(database_id=db_id, table_id="trilhas", key=key, size=size, required=required)
        elif tipo == "integer":
            db.create_integer_column(database_id=db_id, table_id="trilhas", key=key, required=required, min=0, max=1000)
        print(f"  Coluna '{key}' adicionada a 'trilhas'.")
    except Exception:
        pass

# ──────────────────────────────────────────────
# 3. Colunas da tabela 'desafios'
# ──────────────────────────────────────────────
colunas_desafios = [
...
for key, tipo, size, required in colunas_desafios:
    try:
        db.create_string_column(database_id=db_id, table_id="desafios", key=key, size=size, required=required)
        print(f"  Coluna '{key}' adicionada a 'desafios'.")
    except Exception:
        pass

# ──────────────────────────────────────────────
# 4. Colunas da tabela 'certificados'
# ──────────────────────────────────────────────
...
for key, tipo, size, required in colunas_certificados:
    try:
        db.create_string_column(database_id=db_id, table_id="certificados", key=key, size=size, required=required)
        print(f"  Coluna '{key}' adicionada a 'certificados'.")
    except Exception:
        pass

print("Aguardando indexacao das colunas no Appwrite...")
time.sleep(5)

# ──────────────────────────────────────────────
# 5. Popular tabela 'trilhas' — 30 trilhas DIO
# ──────────────────────────────────────────────
trilhas_data = [
...
for t in trilhas_data:
    try:
        db.create_row(database_id=db_id, table_id="trilhas", row_id=ID.unique(), data=t)
        sucesso += 1
    except Exception as e:
        print(f"Erro ao inserir '{t.get('titulo')}': {e}")
